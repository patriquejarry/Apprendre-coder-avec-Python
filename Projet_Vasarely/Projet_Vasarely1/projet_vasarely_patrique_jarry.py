"""
    ...
    formés avec des losanges de couleurs différentes, déformés par une sphère.
    Entrée :
        Les positions inférieur gauche et supérieur droit qui delimitent le tableau.
        La longueur d'une arête de l'hexagone.
        Les couleurs des trois losanges.
        Les coordonnées (abscisse, ordonnée, hauteur) du centre de la sphère.
...
def face(couleur, points, centre, rayon):
    ...
    trace(points[1][0], points[1][1], centre, rayon)  # trace les bords du losange
    trace(points[2][0], points[2][1], centre, rayon)
    trace(points[3][0], points[3][1], centre, rayon)
    trace(points[0][0], points[0][1], centre, rayon)  # boucle le losange au point initial
...
# Hexagones
# Longueur de l'arête et couleurs de chaque face
p_longueur = int(input( "Longueur d'une arête            (int) : "))
p_col1 = input("Couleur 1                             : ")
p_col2 = input("Couleur 2                             : ")
p_col3 = input(        "Couleur 3                             : ")

# Sphère
# Coordonnées tridimensionnel et rayon de la Sphère
p_centre_x = int(input("Abscisse du centre de la sphère (int) : "))
p_centre_y = int(input("Ordonnée du centre de la sphère (int) : "))
p_centre_z = int(input("Hauteur du centre de la sphère  (int) : "))
p_rayon = int(input("Rayon de la sphère              (int) : "))
...
turtle.hideturtle()
turtle.getcanvas().postscript(file="projet_vasarely_patrique_jarry.eps")
turtle.done()

# Galerie
# https://www.fun-mooc.fr/media/15878075415825495.jpg
# Pavé réalisé avec très petits hexagones d'arête taille 3.
# Une journée entière pour le dessiner.
# A la fin, la tortue était encore plus lente qu'un escargot.
